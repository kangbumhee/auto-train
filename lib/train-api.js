...
export async function createReservationId(cookie) {
  const result = await api("/train-reservation-id", {
    body: {},
    cookie: formatCookie(cookie),
  });

  const reserveId =
    result?.res?.reserveId ||
    result?.res?.reservationId ||
    result?.data?.reserveId ||
    result?.data?.reservationId ||
    result?.reserveId ||
    result?.reservationId;

  return { reserveId, raw: result };
}
...
/** 예약 ID 추출 ({ reserveId, raw } 또는 원시 응답) */
export function pickReservationId(result) {
  if (result?.reserveId) return result.reserveId;
